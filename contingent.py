...
def izm_param(fac_old,fac_new,fo_old,fo_new,fin_old,fin_new,flow):
    ''''
       метод анализует была ли смена института, формы обучения или формы финансирования.
       Используется для отсечения приказов с типом перевод, при которых не было смены института,
        формы оучения и финансирования
    Параметры:
...
def get_flow_name(code_pr):
    ''''
    метод получает тип события тандем по коду выписки. Если требуемая выписка не выходит у студента,
     ее код нужно добавить в нужный справочник метода.
    Параметры:
...
def get_contingent_flows(self, guid):
    res = list()
    # Зачисление
    sql = """select pr.id from
    personrole_t AS pr 
    WHERE pr.guid_p='%s'"""
    cur = self.con.cursor()
...
#        for fl in dan_prikaz_st[:]:
#            print(fl)
    return dan_prikaz_st

try:
    connection1 = psycopg2.connect(user="nsn001",        # Подключение к базе данных Тандема
								  password="esz7jba1cm",
								  host="192.168.25.101",
								  port="5432",
								  database="tdmdb")

    connection2 = psycopg2.connect(user="donsitest",        # Подключение к базе данных Дублера
